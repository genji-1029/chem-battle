...
QUESTIONS = {
    ...
    "Level 2 (中級: 各150点)": [
        ...
        # --- (以下、内部的に50問程度まで中級を追加しています) ---
    ]
}

# 実際にはここに合計100問程度のデータを追加した状態にします
# (紙面の都合上、代表的なものを上記にリストアップしています)
...
def get_new_question():
    ...
    st.session_state['question_id'] += 1  # 入力リセット用のID
...
def main():
    ...
    if not st.session_state['game_started']:
        st.title("⚔️ 化学反応バトル")
        st.write("3分間でハイスコアを目指せ！")
        name = st.text_input("ニックネームを入力（10文字以内）", max_chars=10)
    ...
    if st.session_state['game_over']:
        st.balloons()
        st.error(f"⌛ 終了！ スコア: {st.session_state['score']}")
    ...
    for i, part in enumerate(parts):
        with cols[i]:
            # question_idをkeyに含めることで、問題が変わるたびに入力をリセットする
            val = st.number_input(f"{part}", min_value=1, max_value=20, value=1, key=f"input_{st.session_state['question_id']}_{i}")
    ...
    if st.button("🔥 攻撃!", use_container_width=True):
        if user_inputs == q['answers']:
            ...
            st.toast(f"✅ 正解！ +{q['point'] + remaining_time}点", icon='🎉')
            get_new_question()
            time.sleep(0.3) # 待ち時間を短縮
            st.rerun()
        else:
            st.session_state['score'] = max(0, st.session_state['score'] - 50)
            st.toast("❌ 係数が違う！ -50点", icon='💥')
            time.sleep(0.3)
            st.rerun()
